...
then
  ...
  echo "<base_service_name>   Base name of new service principal"
...
do
./krb_princ_create.sh $TEST_SERVICENAME$i/krb.localhost $TEST_PASSWORD
done
...
do
  ...
  ./keytab_create.sh $TEST_SERVICENAME$i/krb.localhost $filepath
done
